...
def score_neighbors_multi_query(
    ...
    for neighbor_id in neighbor_ids:
        ...
        combined_score = sum(
            score * weight for score, weight in zip(individual_scores, query_weights)
        )
...
def recursive_graph_walk(
    ...
    Args:
        ...
        use_mmr: Whether to use MMR for diversity-aware selection
        mmr_lambda: MMR parameter balancing relevance vs diversity (0-1)
        mmr_adjacent_k: Max neighbors to consider per MMR iteration
    ...
    if use_mmr:
        ...
        from .mmr import mmr_graph_walk

        # Get query embedding
        try:
            response = ollama.embeddings(
                model="nomic-embed-text", prompt=original_prompt
            )  # type: ignore[no-untyped-call]
            query_embedding = np.array(response.get("embedding", []))
            ...
            return mmr_graph_walk(
                config=config,
                seed_results=seed_results,
                query_embedding=query_embedding,
                lambda_mult=mmr_lambda,
                select_k=max_hops * 3,  # Allow more selections than hops
                adjacent_k=mmr_adjacent_k,
                max_depth=max_hops,
                min_mmr_score=score_threshold,
            )
        except Exception as e:
            raise RuntimeError(f"Failed to run MMR graph walk: {e}")

    # Default parameters for original walk
    ...
    for hop in range(max_hops):
        ...
        best_neighbor_id = max(
            neighbor_scores.keys(), key=lambda x: neighbor_scores[x][0]
        )
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT id, title, content FROM vault WHERE id = ?", (best_neighbor_id,)
        )
        row = cursor.fetchone()
        conn.close()
